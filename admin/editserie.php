<?php
  require_once('../db/config.php');
   ...
   try {
            $stmt = $db->prepare('SELECT id, titulo, sinopse, img, acao, comedia, ficcao, terror, policial, animacao, drama, romance, infantil FROM serie WHERE id = :id') ;
...
<body>
  ...
  if(isset($_POST['submit'])){
    ...
    if(!isset($error)){
        ...
        $stmt = $db->prepare('UPDATE filme SET titulo = :titulo, img = :img, sinopse = :sinopse, acao = :acao, comedia = :comedia, ficcao = :ficcao, terror = :terror, policial = :policial, animacao = :animacao, drama = :drama, romance = :romance, infantil = :infantil WHERE id = :id') ;
        $stmt->execute(array(
          ':titulo' => $titulo,
          ':img' => $img,
          ':sinopse' => $sinopse,
