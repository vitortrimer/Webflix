...
<body>
  ...
  if(isset($_POST['submit'])){
    ...
    if(!isset($error)){
        ...
        $stmt = $db->prepare('INSERT INTO episodio (titulo, img, embed, epinum, idserie)
                                    VALUES (:titulo, :img, :embed, :epinum, :idserie)') ;
        $stmt->execute(array(
          ':titulo' => $titulo,
          ':img' => $img,
          ':embed' => $embed,
  ...
  <div class="form-group">
    ...
    <input type="text" class="form-control" name="idserie" id="idserie" value='<?php if(isset($error)){ echo $_POST['idserie'];}?>'  placeholder="ID">
  </div>
  <div class="form-group">
    <label for="epinum">Número do episódio</label>
    <input type="text" class="form-control" name="epinum" id="epinum" value='<?php if(isset($error)){ echo $_POST['epinum'];}?>'  placeholder="00-00">
  </div>
  <div class="form-group">
