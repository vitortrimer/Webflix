<?php
	...
	if(isset($_GET['deletafilme'])){ 
	        $stmt = $db->prepare('DELETE FROM filme WHERE id = :id') ;
	        $stmt->execute(array(':id' => $_GET['deletafilme']));
...
<head>
	...
	<title>Administração</title>
	  ...
	  <script type="text/javascript" src="../js/bootstrap.min.js"></script>
		...
		function deletafilme(id, titulo)
		{
		  if (confirm("Deletar filme " + titulo + "?"))
		  {
		      window.location.href = 'index.php?deletafilme=' + id;
		  }
		}
		</script>
		  ...
		  <tbody>
		    <?php
			    try {
			        ...
			        $stmt = $db->query('SELECT id, titulo, datapost FROM filme ORDER BY id DESC');
			        while($row = $stmt->fetch()){
			            ...
			            <td>
			            	<center>
			            	<a href="editfilme.php?id=<?php echo $row['id'];?>">Editar</a> |
			                <a href="javascript:deletafilme('<?php echo $row['id'];?>','<?php echo $row['titulo'];?>')">Deletar</a>
			                </center>
			            </td>
